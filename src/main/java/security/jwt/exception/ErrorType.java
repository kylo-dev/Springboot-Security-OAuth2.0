...
public enum ErrorType {
    ...
    // 리프레시 토큰이 null 인 경우
    REFRESH_TOKEN_DOES_NOT_EXIST(HttpStatus.NOT_FOUND, "TOKEN-003",
        "리프레시 토큰이 존재하지 않습니다. 쿠키를 확인해 주세요."),

    // 어세스 토큰이 만료되어 인증을 진행하지 못하는 경우
    TOKEN_USER_DOES_NOT_AUTHORIZED(HttpStatus.UNAUTHORIZED, "TOKEN-004",
        "어세스 토큰 만료로 인해 유저 인증단계를 밟을 수 없습니다."),
    ...
    // 이메일이 일치하지 않은 경우
    CHECK_ID_AGAIN(HttpStatus.BAD_REQUEST, "USER-001", "아이디를 다시 한번 확인해주세요"),

    // 비밀번호가 일치하지 않은 경우
    CHECK_PASSWORD_AGAIN(HttpStatus.BAD_REQUEST, "USER-002", "비밀번호를 다시 한번 확인해주세요"),

    // 사용자를 찾을 수 없는 경우
    USER_NOT_FOUND(HttpStatus.NOT_FOUND, "USER-003", "유저가 존재하지 않습니다."),
    ALREADY_EXIST_EMAIL(HttpStatus.BAD_REQUEST, "USER-004", "이미 존재하는 이메일입니다."),

    // 내부 서버 오류
    INTERNAL_SERVER_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "SERVER-001",
        "내부 서버 오류로 인해 요청을 처리할 수 없습니다."),
